...
public class StudentDAO {
    ...
    private static final String INSERT_STUDENT = 
        "INSERT INTO students (user_id, first_name, last_name, date_of_birth, gender, phone, address, city, state, zip_code, country, course_id, enrollment_year) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    private static final String SELECT_STUDENT_BY_ID = 
        "SELECT s.*, u.email, u.username, c.course_name FROM students s JOIN users u ON s.user_id = u.user_id JOIN courses c ON s.course_id = c.course_id WHERE s.student_id = ?";

    private static final String SELECT_STUDENT_BY_USER_ID = 
        "SELECT s.*, u.email, u.username, c.course_name FROM students s JOIN users u ON s.user_id = u.user_id JOIN courses c ON s.course_id = c.course_id WHERE s.user_id = ?";

    private static final String SELECT_ALL_STUDENTS = 
        "SELECT s.*, u.email, u.username, c.course_name FROM students s JOIN users u ON s.user_id = u.user_id JOIN courses c ON s.course_id = c.course_id ORDER BY s.registration_date DESC";

    private static final String SELECT_STUDENTS_BY_STATUS = 
        "SELECT s.*, u.email, u.username, c.course_name FROM students s JOIN users u ON s.user_id = u.user_id JOIN courses c ON s.course_id = c.course_id WHERE s.status = ? ORDER BY s.registration_date DESC";

    private static final String SELECT_STUDENTS_BY_COURSE = 
        "SELECT s.*, u.email, u.username, c.course_name FROM students s JOIN users u ON s.user_id = u.user_id JOIN courses c ON s.course_id = c.course_id WHERE s.course_id = ? ORDER BY s.registration_date DESC";

    private static final String SELECT_STUDENTS_BY_YEAR = 
        "SELECT s.*, u.email, u.username, c.course_name FROM students s JOIN users u ON s.user_id = u.user_id JOIN courses c ON s.course_id = c.course_id WHERE s.enrollment_year = ? ORDER BY s.registration_date DESC";

    private static final String UPDATE_STUDENT = 
        "UPDATE students SET first_name = ?, last_name = ?, date_of_birth = ?, gender = ?, phone = ?, address = ?, city = ?, state = ?, zip_code = ?, country = ?, course_id = ?, enrollment_year = ? WHERE student_id = ?";

    private static final String UPDATE_STUDENT_STATUS = 
        "UPDATE students SET status = ?, approved_by = ?, approved_date = ?, rejection_reason = ? WHERE student_id = ?";

    private static final String DELETE_STUDENT = 
        "DELETE FROM students WHERE student_id = ?";

    private static final String COUNT_STUDENTS_BY_STATUS = 
        "SELECT COUNT(*) FROM students WHERE status = ?";

    private static final String SEARCH_STUDENTS = 
        "SELECT s.*, u.email, u.username, c.course_name FROM students s JOIN users u ON s.user_id = u.user_id JOIN courses c ON s.course_id = c.course_id WHERE s.first_name LIKE ? OR s.last_name LIKE ? OR u.email LIKE ? OR u.username LIKE ? ORDER BY s.registration_date DESC";
    ...
    public Student getStudentById(int studentId) {
        ...
        try {
            ...
            if (resultSet.next()) return extractStudentFromResultSet(resultSet);
    ...
    public List<Student> getStudentsByYear(int year) {
        ...
        try {
            connection = DBConnection.getConnection();
            statement = connection.prepareStatement(SELECT_STUDENTS_BY_YEAR);
